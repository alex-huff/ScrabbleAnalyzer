...
interface IScrabbleBoard
{
    ...
    default
    boolean isEqualTo(IScrabbleBoard otherBoard)
    {
        return this.getRows() == otherBoard.getRows() && this.getCols() == otherBoard.getCols() && this.trueForAllTiles(
            (r, c) -> this.getLetterMultiplierAt(r, c) == otherBoard.getLetterMultiplierAt(r, c)) &&
               ...
               this.getAnchorRow() == otherBoard.getAnchorRow() && this.getAnchorCol() == otherBoard.getAnchorCol();
    }

    private
    boolean trueForAllTiles(BiPredicate<Integer, Integer> tileCondition)
    {
        return IntStream.range(0, this.getRows())
            .allMatch(r -> IntStream.range(0, this.getCols()).allMatch(c -> tileCondition.test(r, c)));
    }
